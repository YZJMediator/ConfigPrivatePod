...
DevelopVersionString=`grep -E 'version.develop.*=' __ProjectName___Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' __ProjectName___Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' __ProjectName___Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' __ProjectName___Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" __ProjectName___Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' __ProjectName___Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" __ProjectName___Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' __ProjectName___Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" __ProjectName___Extension.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' __ProjectName___Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" __ProjectName___Extension.podspec
...
pod repo push ${Repo} __ProjectName___Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
